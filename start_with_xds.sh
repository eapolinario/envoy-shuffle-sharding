#!/bin/bash
# Start xDS control plane, backends, and Envoy with RTDS support
...
echo "====================================================="
echo "Starting Envoy Shuffle Sharding with xDS/RTDS"
echo "====================================================="
echo ""
...
echo ""
echo "2. Starting 8 backend servers..."
for i in {1..8}; do
...
# Start Envoy with xDS config
echo ""
echo "3. Starting Envoy with RTDS support..."
docker stop envoy-shuffle-shard 2>/dev/null || true
docker rm envoy-shuffle-shard 2>/dev/null || true
...
docker run -d \
    ...
    -p 10000:10000 \
    -p 9901:9901 \
    -v "$(pwd)/envoy-xds.yaml:/etc/envoy/envoy.yaml:ro" \
...
if docker ps | grep -q envoy-shuffle-shard; then
    ...
    echo "  - Backend servers:   localhost:8001-8008"
